  ...
  $.each(userNames, function (index, value) {  //build DOM from API results sending userNames array
    $.getJSON('https://api.twitch.tv/kraken/streams/' + value + '?client_id=lwqoscrxiee347pi183ialxp5z7r8rq&callback=?', function (data) {
      ...
      if (online === "Offline") {
        ...
        $('.Grid').append('<div class="grid-items"><div class="mini-grid green-active"><div class="content-header"><a href="https://www.twitch.tv/' + value + '" target="blank">' + value + '<a/><img src="' + data.stream.preview.small + '"/></div><div class="content-item"><p>' + data.stream.game + ': '  + data.stream.channel.status + '</p></div></div></div>');
      }
